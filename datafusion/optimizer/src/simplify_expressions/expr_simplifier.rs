...
impl<S: SimplifyInfo> TreeNodeRewriter for Simplifier<'_, S> {
    ...
    fn f_up(&mut self, expr: Expr) -> Result<Transformed<Expr>> {
        ...
        Ok(match expr {
            ...
            //
            // Safe Arithmetic Comparison Optimizations (with overflow protection)
            //

            // Simplify: a + b < a -> false (ONLY if safe from overflow)
            // This is the conservative approach: optimize to false only when safe
            Expr::BinaryExpr(BinaryExpr {
                left,
                op: Operator::Lt,
                right,
            }) => {
                println!("DEBUG: Checking Lt BinaryExpr: left={:?}, right={:?}", left, right);
                if is_simple_addition(&left) && expressions_equal_except_constant(&left, &right) {
                    println!("DEBUG: Pattern matched: a + const < a");
                    ...
                    // This is the key change: continue to check the other pattern
                    if is_simple_addition(&right) && expressions_equal_except_constant(&left, &right) {
                        println!("DEBUG: Pattern matched: a < a + const");
                        if let Some(constant) = extract_addition_constant(&right) {
                            println!("DEBUG: Extracted constant: {}", constant);
                        ...
                        println!("DEBUG: Second pattern not matched");
                        Transformed::no(Expr::BinaryExpr(BinaryExpr { left, op: Operator::Lt, right }))
                    }
                }
            }
...
/// Check if the expression is a simple addition: a + constant or constant + a
fn is_simple_addition(expr: &Expr) -> bool {
    matches!(
        expr,
        Expr::BinaryExpr(BinaryExpr {
            op: Operator::Plus,
...
/// Check if two expressions are equal except for a constant addition
/// e.g., returns true for ("a + 1", "a") or ("a", "1 + a")
fn expressions_equal_except_constant(expr1: &Expr, expr2: &Expr) -> bool {
    // Helper to check if a binary expression (op +) contains the target expression
    let check_binary = |binary: &BinaryExpr, target: &Expr| -> bool {
        if binary.op != Operator::Plus {
            return false;
        }
        // Check left side (operand + constant)
        if binary.left.as_ref() == target {
            return matches!(binary.right.as_ref(), Expr::Literal(..));
        }
        // Check right side (constant + operand)
        if binary.right.as_ref() == target {
            return matches!(binary.left.as_ref(), Expr::Literal(..));
        }
        false
    };
    ...
    match (expr1, expr2) {
        // Case: (A + Const) vs A
        (Expr::BinaryExpr(b), target) => check_binary(b, target),
        // Case: A vs (A + Const)
        (target, Expr::BinaryExpr(b)) => check_binary(b, target),
        _ => false,
    }
}

/// Extract constant value from addition expression
fn extract_addition_constant(expr: &Expr) -> Option<i64> {
    match expr {
        Expr::BinaryExpr(BinaryExpr {
            left,
            op: Operator::Plus,
            ...
            // Check right side first (common case: col + 1)
            if let Expr::Literal(val, _) = right.as_ref() {
                return get_int_scalar_value(val);
            }
            // Check left side (case: 1 + col)
...
/// Helper to extract i64 from various integer ScalarValues
fn get_int_scalar_value(val: &ScalarValue) -> Option<i64> {
    match val {
        ...
        ScalarValue::UInt64(Some(v)) => match i64::try_from(*v) {
            Ok(v_i64) => Some(v_i64),
            Err(_) => None, // Ignore u64 that doesn't fit in i64
        },
        _ => None,
    }
}
